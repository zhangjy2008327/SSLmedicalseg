...
from RegSeg_10 import RegSeg
from torch.utils.data import DataLoader
from sklearn.model_selection import KFold
import albumentations as A

from MyDataLoder import Dataset
...
def main():
    ...
    for fold, (train_indices, val_indices) in enumerate(kfold.split(img_ids)):
        ...
        encoder_weights_path = './checkpoint/RegSeg_10_brats18_encoder.pth'
        ...
        best_val_metric = 0
        best_epoch = 0
        train_logs = {'epoch': [], 'loss': [], 'acc': [], 'pre': [], 'rec': [], 'Miou': [], 'dice': [], 'HD': [],
                      'm_b_iou': [], 'MAE': []}
        val_logs = {'epoch': [], 'acc': [], 'pre': [], 'rec': [], 'Miou': [], 'dice': [], 'HD': [], 'm_b_iou': [],
                    'MAE': []}
        test_logs = {'fold': [], 'acc': [], 'pre': [], 'rec': [], 'Miou': [], 'dice': [], 'HD': [], 'm_b_iou': [],
                     'MAE': []}
        ...
        for epoch in range(args.epochs):  # 假设每个折进行10轮迭代
            ...
            training_mae = 0
            training_num = 0
            for idx, images, masks in train_loader:
            ...
            val_num = 0
            with torch.no_grad():
                for idx, images, masks in val_loader:
        ...
        idx=[]
        with torch.no_grad():
            save_path = '/nvme1n1_7t1/wy/RegSeg/brats18/'
            dir = os.path.join(save_path,f"fold_{fold+1}")
            os.makedirs(dir, exist_ok=True)
            for idx, images, masks in test_loader:
                ...
                ########################每一个跑出来有十个文件，对应十折，灭个文件里又有每一层的效果图
                #predicted_path = os.path.join(save_folder, f'predicted_mask_{i}.png')
                #masks_path = os.path.join(save_folder, f'ground_truth_mask_{i}.png')
                ...
                for i in range(args.batch_size):
                    if i<len(predicted):
                        ...
                        plt.subplot(1, 2, 1)
                        plt.imshow(predicted[i, 0], cmap='gray')#这里之前不加if的时候会报错，可能时最后一个batch不够8，分成了4，所以下标会越界，这里的predicted[i,0]表示每个batch中的第i个图的第一个通道
                        plt.axis('off') 
                        plt.savefig(f"{dir}/{idx[i]}_predicted.png",bbox_inches='tight',pad_inches=0,transparent=True)
                        plt.cla()#因为这个过程中会创建很多figure对象，会收到警告，使用这个代码能清除figure对象
                        plt.close("all")
                        ...
                        plt.axis('off') 
                        plt.savefig(f"{dir}/{idx[i]}_mask.png",bbox_inches='tight',pad_inches=0,transparent=True)
                        plt.cla()
                        plt.close("all")
                    #plt.savefig(masks_path)
                
                
                #plt.figure()
                #plt.imshow(predicted)
                #plt.axis('off')
                #plt.savefig(f"{dir}/{idx}_predicted.png")
                #plt.figure()
                #plt.imshow(predicted)
                #plt.axis('off')
                #plt.savefig(f"{dir}/{idx}_mask.png")
